...
const AdminProjectsContainer = ({
    ...
    if (projects) {
      ...
      if (statusFilters) {
        filtered = filtered.filter((project) => {
          if (statusFilters.isActive && project.status === "active")
            return true;
          if (statusFilters.isInactive && project.status === "inactive")
            return true;
          if (statusFilters.isPending && project.status === "pending")
            return true;
          return false;
        });
      }
  ...
  }, [projects, statusFilters, sortType, searchQuery, SortingTypes]);
          ...
          >
            ...
            <div className="flex items-center justify-between gap-5 p-2">
              ...
              <div className="flex flex-col">
                <h2 className="mb-2 text-sm font-semibold">{project.projectName}</h2>
            ...
            <Button
              ...
              onClick={() => {
                navigation(`/admin/details/${project._id}`);
              }}
            />
          </Board>
        ))
